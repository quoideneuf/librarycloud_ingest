...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="mods:mods">
      <xsl:message>New Doc</xsl:message>
        <xsl:element name="doc">
            ...
            <xsl:element name="field">
              ...
              <xsl:call-template name="buildDateRange">
                <!-- <xsl:with-param name="dateNodes" select="(descendant::*[local-name()='dateIssued' or local-name()='dateCreated'])"/> -->
              </xsl:call-template>
...
<!--     <xsl:template match="*" mode="hierarchicalGeographic">
 ...
 -->
    ...
    <xsl:template name="buildDateRange">
      ...
      <!-- <xsl:message> -->
      <!--   Position: <xsl:value-of select="$position" /> Count: <xsl:value-of select="count($dateNodes)" /> -\-Low <xsl:value-of select="$lowDate" /> High <xsl:value-of select="$highDate" /> -->
      <!--   <xsl:value-of select="($dateNodes[number($position)])" /> -->
      <!-- </xsl:message> -->



      <!-- <xsl:message> -->
      <!--   <xsl:value-of select="$dateNodes" /> -->
      <!-- </xsl:message> -->
      <xsl:choose>
        <xsl:when test="count($dateNodes[number($position)]) &gt; 0">
          ...
          <!-- <xsl:message> -->
          <!--   <xsl:value-of select="concat($currentDateNode, ' - ', $dateString)" /> -->
          <!-- </xsl:message> -->

          <xsl:call-template name="buildDateRange">
            <xsl:with-param name="lowDate">
              <xsl:choose>
                <xsl:when test="number(substring($dateString, 1, 4)) &lt; $lowDate">
                  <xsl:value-of select="number(substring($dateString, 1, 4))" />
                </xsl:when>
                <xsl:otherwise>
            ...
            <xsl:with-param name="highDate">
              <xsl:choose>
                <xsl:when test="string-length($dateString) = 9 and number(substring($dateString, 6, 4)) &gt; $highDate">
                  <xsl:value-of select="number(substring($dateString, 6, 4))" />
                </xsl:when>
                <xsl:when test="string-length($dateString) = 4 and number($dateString) &gt; $highDate">
        ...
        <xsl:otherwise>
          <xsl:message>Result: <xsl:value-of select="concat(string($lowDate), ' TO ', string($highDate)) " /></xsl:message>
    ...
    <xsl:template name="normalizeDate">
      ...
      <xsl:param name="dateStringOutput" select="'ONCE'"/>
      <!-- <xsl:message> -->
      <!--   <xsl:value-of select="concat('Input: ', $dateStringInput)" /> -->
      <!-- </xsl:message> -->
      <!-- <xsl:message> -->
      <!--   <xsl:value-of select="concat('Output: ', $dateStringOutput)" /> -->
      <!-- </xsl:message> -->


      <xsl:choose>
        <xsl:when test="$dateStringOutput = 'ONCE'">
          <xsl:call-template name="normalizeDate">
            <xsl:with-param name="dateStringInput" select="translate(translate($dateStringInput, ' ', ''), 'or', '-')" />
            <xsl:with-param name="dateStringOutput" select="''" />
          </xsl:call-template>
        </xsl:when>
        ...
        <!-- <xsl:when test="string-length($dateStringInput) = 9 and substring($dateStringInput, 5, 1) = '-'"> -->
        <!--   <xsl:call-template name="normalizeDate"> -->
        <!--     <xsl:with-param name="dateStringInput" select="substring($dateStringInput, '-?', '0')" /> -->
        <!--     <xsl:with-param name="dateStringOutput" select="$dateStringOutput" /> -->
        <!--   </xsl:call-template> -->
        <!-- </xsl:when> -->
        <xsl:otherwise>
